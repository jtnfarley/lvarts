...
const PostForm = ({user, edited, savePost, post, onAudioFileSelected, addToRadio}: Props) => {
    ...
    const [venueQuery, setVenueQuery] = useState(post?.venuename ?? '');
    ...
    const eventid = post?.eventid ?? undefined;
    const eventname = post?.eventname ?? '';
    const eventdate = post?.eventdate ?? undefined;
    const venuename = post?.venuename ?? '';
    const address = post?.address ?? '';
    const venueid = post?.venueid ?? undefined;
